...
export const addItem = async (req: Request, res: Response) => {
  const { name, price, count, owner } = req.body;
  const newItem = await itemModel.addItem(name, price, count, owner);
  res.status(201).json(newItem);
};

export const editItem = async (req: Request, res: Response) => {
  const { name, price, count, owner  } = req.body;
  const updatedItem = await itemModel.updateItem((req.params.id), name, price, count, owner);
