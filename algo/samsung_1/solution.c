...
int N, K; // 신호 길이, 암호 길이 (전역)

int *signal_arr;   // 전역 포인터
int *password_arr; // 전역 포인터
...
void input()
{
	...
	signal_arr = (int *)malloc(sizeof(int) * N);
	password_arr = (int *)malloc(sizeof(int) * K);

	for (int i = 0; i < N; i++)
		scanf("%d", &signal_arr[i]);

	for (int i = 0; i < K; i++)
		scanf("%d", &password_arr[i]);
}
...
int solve()
{
	...
	for (int i = 0; i < N; i++)
	{
		if (password_arr[j] == signal_arr[i])
		{
			j++;
			if (j == K)
			{
