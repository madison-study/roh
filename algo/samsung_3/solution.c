...
int get_first()
{
    ...
    for (int i = 0; i < population; i++)
    {
        ...
        while (removed[votes[i * population + top[i]] - 1])
        {
            top[i]++;
        }

        int cand = votes[i * population + top[i]] - 1;
        count[cand]++;
    }

    for (int i = 0; i < population; i++)
    {
        if (count[i] > population / 2)
            return i + 1;
    }
...
int remove_process()
{
    ...
    for (int i = 0; i < population; i++)
    {
        ...
        while (removed[votes[i * population + bottom[i]] - 1])
        {
            bottom[i]--;
        }

        int cand = votes[i * population + bottom[i]] - 1;
        count[cand]++;
    }
    ...
    for (int i = 0; i < population; i++)
    {
        if (!removed[i] && count[i] == max)
        {
            remove_candidate(i + 1);
            active--;
        }
    }
...
void input()
{
    scanf("%d", &population);
    active = population;
    top = malloc(sizeof(int) * population);
    bottom = malloc(sizeof(int) * population);
    removed = calloc(population, sizeof(int));
    votes = malloc(sizeof(int) * population * population);
    for (int i = 0; i < population; i++)
    {
        top[i] = 0;
        bottom[i] = population - 1;
        for (int j = 0; j < population; j++)
        {
            scanf("%d", &votes[i * population + j]);
        }
    }
}
